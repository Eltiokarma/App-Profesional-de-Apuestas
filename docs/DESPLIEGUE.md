...
3. **Variables** (`Settings → Variables`):
   ...
   | `SAD_DT_AGENDA_DIAS` | `2` | (opcional) DT fresco de los equipos que juegan en ≤ N días: alineación del último partido + `/coachs` si el registro tiene más de `SAD_DT_AGENDA_EDAD_DIAS` (7) o la alineación lo contradice. ≤ 2 requests por equipo, en la corrida diaria (`jugadores --dt-agenda`) |
   | `SAD_JUGADORES_SOLO_DT` | *(vacía)* | (opcional, **de una sola corrida**) `1` hace que la ingesta de jugadores rehaga SOLO el entrenador de cada equipo con partido próximo, a 1 request por equipo y sin esperar el TTL de 30 días. Para después de cambiar la regla del DT vigente (16/09/2026: la base conservaba al saliente en 17 de 22 equipos). Ponerla, esperar una corrida programada, y quitarla: mientras esté, cada corrida gasta esos requests |
