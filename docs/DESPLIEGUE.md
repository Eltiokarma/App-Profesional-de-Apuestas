...
3. **Variables** (`Settings → Variables`):
   ...
   | `SAD_BLOQUE_RESERVA_PARTIDOS` | `3500` | (opcional) reserva de la ingesta EN BLOQUE (backfill, jugadores, ficha) **cuando hay fútbol nuestro cerca**. Tiene que ser bastante MAYOR que `SAD_LIVE_RESERVA`: si son iguales, el bloque gasta hasta dejar exactamente el suelo del ciclo en vivo y el vivo amanece apagado — margen real CERO. Eso pasó el 30/07/2026: 309 ciclos con las cuotas en juego en pausa, 5 h 30 min de prime time, y el plan ni se agotó (6.670/7.495) |
   ...
   | `SAD_LIVE_RESERVA` | `1500` | (opcional) requests del plan diario que el ciclo en vivo NO puede tocar. Es el freno que impide que un ciclo por minuto vacíe el plan y deje sin datos a la ingesta diaria. Al acercarse a la reserva el cupo por ciclo se encoge solo; al llegar, el ciclo sigue pidiendo marcador y minuto (1 request) pero deja de pedir cuotas |
   | `SAD_LIVE_BACKOFF_PASO_MIN` / `_TOPE` | `3` / `10` | (opcional) backoff por liga: cada ronda SEGUIDA sin datos aleja la siguiente consulta `paso × rondas` minutos, con tope. Una liga que la API no cubre pasa de consultarse cada ciclo a cada 30 min, y el cupo liberado va a las que sí dan cuotas. La auditoría del 31/07/2026: 506 rondas vacías + 290 rescates con CERO aciertos = 796 requests tiradas (11% del plan) en confirmar 115 veces la misma noche que no hay cobertura de Bolivia. Una ronda con datos resetea el contador; un `fallo` de red NO castiga |
