...
| **Gap de regresión** | `μ(puntos esperados) − forma reciente`. **`gap > 0` = rinde por DEBAJO de su nivel** (tiende a mejorar); `gap < 0` = por encima. Ver `references/ley_regresion_nivel.md` |
...
- `references/ley_regresion_nivel.md` → cálculo de nivel, Gap, discretización en bins
...
- **Regresión:** `|Gap| ≥ 0.8` = señal estructural. **No predice goles ni marcador.**
  Signo: `gap > 0` = subrinde (tiende a mejorar). Reportar el gap **clásico y el
  ajustado por calendario**; la diferencia entre ambos es el efecto del calendario.
  Horizonte **~5 partidos**, no el partido de hoy (backtest: al siguiente partido
  el efecto es ≤0.1 pts). Detalle en `references/ley_regresion_nivel.md`.
